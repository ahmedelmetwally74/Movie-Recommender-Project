...
def get_user_based_recommendations(user_id, num_recommendations=24):
    model_path = DATA_PATH.joinpath("cf_model_final.keras")
...
def get_item_based_recommendations(user_id, num_recommendations=24):
    model_path = DATA_PATH.joinpath("movie_recommender.h5")
    item_model = load_model(model_path)
...
def update_recommended_movies(pathname):
    if pathname and pathname.startswith('/pages/recommendations?user_id='):
        ...
        try:
            ...
            heading = dbc.Row([
                dbc.Col(html.H3("Recommended for this Movie"), width={"size": 6}, style={"margin-top": "40px", 'margin-left': '10px'}),
                dbc.Col(html.Hr())
            ])
